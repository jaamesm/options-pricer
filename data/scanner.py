...
def scan(
    ...
    chain = chain[chain["expiry"] >= 7/365].copy()   # exclude < 1 week

    # Filter to near-the-money contracts only (moneyness within 10% of spot)
    chain = chain[abs(chain["strike"] / chain["S"] - 1) <= 0.10].copy()
